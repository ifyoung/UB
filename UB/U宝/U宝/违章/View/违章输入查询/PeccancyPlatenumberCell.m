...
@interface PeccancyPlatenumberCell ()<UITextFieldDelegate>{
...
@implementation PeccancyPlatenumberCell

+ (instancetype)cellWithTableView:(UITableView *)tableView{
    
    static NSString *ID = @"PeccancyPlatenumberCell";
    PeccancyPlatenumberCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[PeccancyPlatenumberCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)layoutSubviews{
    ...
    rightLabel.text = [SeekparamModel shareInstance].cityNickName;
    
    textF.text = [SeekparamModel shareInstance].plateNo;
}
...
- (void)textFieldWithText:(UITextField *)textField{
    ...
    [SeekparamModel shareInstance].plateNo = [NSString stringWithFormat:@"%@",textField.text];
}
@end
